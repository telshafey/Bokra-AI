...
import { OffboardingTemplate, OffboardingProcess } from '../types';
import { PlusCircleIcon, PencilIcon, TrashIcon, UserGroupIcon, ClipboardDocumentCheckIcon } from './icons/Icons';
import OffboardingTemplateModal from './OffboardingTemplateModal';
import PageHeader from './PageHeader';

interface OffboardingTemplatesPageProps {
    offboardingTemplates: OffboardingTemplate[];
    offboardingProcesses: OffboardingProcess[];
    onSaveTemplate: (template: OffboardingTemplate) => void;
    onDeleteTemplate: (templateId: string) => void;
}

const OffboardingTemplatesPage: React.FC<OffboardingTemplatesPageProps> = ({
    ...
    const usageCountMap = useMemo(() => {
        return offboardingProcesses.reduce((acc, process) => {
    ...
    const handleOpenEditModal = (template: OffboardingTemplate) => {
    ...
    return (
        <div className="space-y-6">
            <PageHeader 
                title="إدارة قوالب إنهاء الخدمة"
                subtitle="إنشاء وتخصيص خطط المغادرة للمناصب المختلفة."
