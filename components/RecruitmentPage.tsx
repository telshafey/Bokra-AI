...
const STAGE_COLORS: Record<CandidateStage, string> = {
    Applied: 'border-slate-500 text-slate-500',
    Screening: 'border-sky-500 text-sky-500',
    Interview: 'border-purple-500 text-purple-500',
    Offer: 'border-amber-500 text-amber-500',
    Hired: 'border-emerald-500 text-emerald-500',
...
const RecruitmentPage: React.FC<RecruitmentPageProps> = ({ jobOpenings, candidates, onUpdateCandidateStage }) => {
    ...
    const handleDrop = (e: React.DragEvent<HTMLDivElement>, newStage: CandidateStage) => {
        ...
        (e.currentTarget as HTMLDivElement).classList.remove('bg-slate-200');
    };
    ...
    const renderKanbanBoard = () => {
        if (!selectedJobId) {
            return <div className="text-center p-12 text-slate-500">الرجاء اختيار وظيفة لعرض المتقدمين.</div>;
        }
        ...
        return (
            <div className="flex gap-4 overflow-x-auto p-4 bg-slate-50 rounded-lg min-h-[60vh]">
                {stages.map(stage => (
                    <div 
                        key={stage}
                        className="w-72 bg-slate-100 rounded-lg flex-shrink-0 flex flex-col"
                        onDrop={(e) => handleDrop(e, stage)}
                        onDragOver={handleDragOver}
                        onDragEnter={(e) => (e.currentTarget as HTMLDivElement).classList.add('bg-slate-200')}
                        onDragLeave={(e) => (e.currentTarget as HTMLDivElement).classList.remove('bg-slate-200')}
                    >
                        <h3 className={`font-bold text-slate-800 p-3 border-t-4 ${STAGE_COLORS[stage]}`}>
                        ...
                        <div className="space-y-3 p-2 h-full overflow-y-auto">
                            {currentCandidates.filter(c => c.stage === stage).map(candidate => (
                                <div 
                                    key={candidate.id} 
                                    className="bg-white p-3 rounded-md shadow-sm cursor-grab active:cursor-grabbing border-l-4 border-slate-300"
                                    draggable
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                {jobOpenings.map(job => {
                    ...
                    <button 
                        ...
                        className={`p-5 bg-white rounded-xl shadow-md text-right transition-all duration-300 hover:shadow-lg hover:-translate-y-1 border-2 ${selectedJobId === job.id ? 'border-sky-500 ring-2 ring-sky-200' : 'border-transparent'}`}
                    >
                        <div className="flex justify-between items-start">
                            <div>
                                <h2 className="text-lg font-bold text-slate-800">{job.title}</h2>
                                <p className="text-sm text-slate-500 my-1">{job.department}</p>
                            </div>
                            <span className={`px-2 py-0.5 text-xs font-semibold rounded-full ${job.status === 'Open' ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-100 text-slate-600'}`}>
                                {job.status === 'Open' ? 'مفتوحة' : 'مغلقة'}
                            </span>
                        </div>
                         <div className="flex items-center text-sm text-slate-600 pt-3 mt-3 border-t">
                            <UsersIcon className="w-5 h-5 ml-2 text-sky-600"/>
                            <span>إجمالي المتقدمين: <strong>{stats.total}</strong></span>
                        </div>
                                ...
                                {pipelineStages.map(stage => {
                                    ...
                                    return (
                                        <div key={stage} className="flex items-center gap-1.5">
                                            ...
                                            <span className="text-slate-600">{STAGE_TRANSLATION[stage]}: <strong>{stats[stage]}</strong></span>
                                        </div>
                                    )
                                })}
                            </div>
                 ...
                 <button className="p-5 bg-white rounded-xl shadow-md flex flex-col items-center justify-center text-center text-slate-500 transition-all duration-300 hover:shadow-lg hover:-translate-y-1 border-2 border-dashed hover:border-sky-500">
