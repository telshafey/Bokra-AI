            ...
            <div className="bg-white p-6 rounded-xl shadow-md">
                 <div className="space-y-6">
                    {Array.from(tasksByCategory.entries()).map(([category, tasks]) => (
                        <div key={category}>
                            <h3 className="font-bold text-slate-700 mb-3 border-b pb-2 text-lg">{t(`onboarding.categories.${category}`)}</h3>
                            <div className="space-y-3">
                                {tasks.map(task => {
                                    const isEmployeeResponsible = task.responsible === 'newEmployee';
                                    return (
                                            ...
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-xs text-slate-500">
                                                        {t('myOnboarding.responsible')}: {t(`onboarding.responsibles.${task.responsible}`)} | {t('myOnboarding.dueDate')}: {task.dueDate}
                                                    </p>
                                                </div>
                                            </div>
                                        </div>
