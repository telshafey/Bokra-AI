...
const HelpCategoryModal: React.FC<HelpCategoryModalProps> = ({ isOpen, onClose, categoryToEdit }) => {
    ...
    return (
        ...
        >
            <div 
                className={`bg-white dark:bg-slate-800 rounded-xl shadow-2xl p-8 w-full max-w-md transform transition-all duration-300 ease-in-out ${isOpen ? 'scale-100 opacity-100' : 'scale-95 opacity-0'}`}
            ...
            >
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        ...
                        <input type="text" value={name.ar} onChange={e => handleChange('ar', e.target.value)} className="w-full p-2 border rounded-lg dark:bg-slate-700 dark:border-slate-600" required spellCheck="true" />
                    </div>
                    <div>
                        ...
                        <input type="text" value={name.en} onChange={e => handleChange('en', e.target.value)} className="w-full p-2 border rounded-lg dark:bg-slate-700 dark:border-slate-600" required spellCheck="true" />
                    </div>
