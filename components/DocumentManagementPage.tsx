...
const DocumentManagementPage: React.FC<DocumentManagementPageProps> = ({ allDocuments, employees, onSaveDocument, onBulkDeleteDocuments }) => {
    ...
    const getStatus = (doc: EmployeeDocument): { text: string; color: string } => {
        if (!doc.expirationDate) {
            return { text: 'سارٍ', color: 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/60 dark:text-emerald-300' };
        }
        const isExpired = new Date(doc.expirationDate) < new Date();
        if (isExpired) {
            return { text: 'منتهي الصلاحية', color: 'bg-red-100 text-red-800 dark:bg-red-900/60 dark:text-red-300' };
        }
        return { text: 'سارٍ', color: 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/60 dark:text-emerald-300' };
    };
            ...
            />
             <ActionBar>
                <div className="flex items-center gap-4">
                    <div>
                        <label className="text-sm font-medium text-slate-600 dark:text-slate-300 mr-2">النوع:</label>
                        <select value={filter.type} onChange={e => setFilter(f => ({...f, type: e.target.value}))} className="p-2 border dark:border-slate-600 rounded-lg text-sm bg-slate-50 dark:bg-slate-700 dark:text-white">
             ...
             {selectedDocIds.size > 0 && (
                <div className="bg-sky-100 dark:bg-sky-900/50 p-3 rounded-lg flex justify-between items-center">
                    <span className="text-sky-800 dark:text-sky-300 font-semibold">{selectedDocIds.size} مستندات محددة</span>
                    ...
                    <table className="w-full text-sm text-right text-slate-500 dark:text-slate-300">
                        ...
                        <tbody>
                            {filteredAndSortedDocuments.map(doc => {
                                ...
                                return (
                                <tr key={doc.id} className="bg-white dark:bg-slate-800 border-b dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700">
                                    ...
                                    <td className="px-6 py-4 flex items-center gap-1">
                                        <button onClick={() => handleOpenEditModal(doc)} className="p-2 text-slate-500 dark:text-slate-400 hover:text-sky-600 dark:hover:text-sky-400 rounded-full" title="تعديل">
                                            <PencilIcon className="w-5 h-5"/>
                                        </button>
                                         <button onClick={() => onBulkDeleteDocuments([doc.id])} className="p-2 text-slate-500 dark:text-slate-400 hover:text-red-600 dark:hover:text-red-500 rounded-full" title="حذف">
