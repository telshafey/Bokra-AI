    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <h1 className="text-2xl font-bold text-slate-800 dark:text-slate-100">{t('pageTitles.externalTasksManagement')}</h1>
                <button
                ...
                >
                    ...
                    <span>{t('externalTasks.assignNew')}</span>
                </button>
            </div>
             ...
             <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-md">
                 <div className="flex items-center gap-4 mb-4 pb-4 border-b dark:border-slate-700">
                    <div>
                        <label className="text-sm font-medium text-slate-600 dark:text-slate-300 mr-2">{t('general.employee')}:</label>
                    ...
                    <div>
                        <label className="text-sm font-medium text-slate-600 dark:text-slate-300 mr-2">{t('general.status')}:</label>
                        <select value={filter.status} onChange={e => setFilter(f => ({...f, status: e.target.value}))} className="p-2 border dark:border-slate-600 rounded-lg text-sm bg-slate-50 dark:bg-slate-700 dark:text-white">
                            <option value="all">{t('general.all')}</option>
                            {Object.keys(STATUS_BADGE_CLASSES).map(s => <option key={s} value={s}>{t(`myTasks.statuses.${s}`)}</option>)}
                        </select>
                    </div>
                </div>
                    ...
                    <table className="w-full text-sm text-right text-slate-500 dark:text-slate-400">
                         <thead className="text-xs text-slate-700 dark:text-slate-300 uppercase bg-slate-100 dark:bg-slate-700">
                             <tr>
                                ...
                                <th className="px-6 py-3">{t('externalTasks.task')}</th>
                                <th className="px-6 py-3">{t('general.date')}</th>
                                <th className="px-6 py-3">{t('general.status')}</th>
                            ...
                            {filteredTasks.map(task => {
                                ...
                                <tr key={task.id} className="bg-white dark:bg-slate-800 border-b dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50">
                                    ...
                                    <td className="px-6 py-4">
                                        ...
                                        {task.requestedById && <span className="text-xs text-slate-500 dark:text-slate-400">({t('externalTasks.requestedByEmployee')})</span>}
                                    </td>
                                    ...
                                    <td className="px-6 py-4">
                                        {task.status === 'PendingApproval' ? (
                                            <div className="flex items-center gap-2">
                                                <button onClick={() => onRequestAction(task.id, 'Reject')} className="p-2 text-red-500 hover:bg-red-100 dark:hover:bg-red-900/50 rounded-full" title={t('general.reject')}><XCircleIcon className="w-5 h-5"/></button>
                                                <button onClick={() => onRequestAction(task.id, 'Approve')} className="p-2 text-emerald-500 hover:bg-emerald-100 dark:hover:bg-emerald-900/50 rounded-full" title={t('general.approve')}><CheckCircleIcon className="w-5 h-5"/></button>
                                            </div>
                                        ) : (
