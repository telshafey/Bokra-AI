    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <h1 className="text-2xl font-bold text-slate-800">إدارة المهام الخارجية</h1>
                <button
             ...
             <div className="bg-white p-6 rounded-xl shadow-md">
                 <div className="flex items-center gap-4 mb-4 pb-4 border-b">
                    <div>
                        ...
                        <select value={filter.employeeId} onChange={e => setFilter(f => ({...f, employeeId: e.target.value}))} className="p-2 border rounded-lg text-sm">
                            <option value="all">الكل</option>
                            {teamMembers.map(e => <option key={e.id} value={e.id}>{e.name}</option>)}
                        </select>
                    </div>
                    <div>
                        <label className="text-sm font-medium text-slate-600 mr-2">الحالة:</label>
                        <select value={filter.status} onChange={e => setFilter(f => ({...f, status: e.target.value}))} className="p-2 border rounded-lg text-sm">
                            <option value="all">الكل</option>
                            ...
                            {filteredTasks.map(task => {
                                ...
                                <tr key={task.id} className="bg-white border-b hover:bg-slate-50">
                                    ...
                                    <td className="px-6 py-4">
                                        ...
                                        {task.requestedById && <span className="text-xs text-slate-500">(طلب من الموظف)</span>}
                                    </td>
                                    ...
                                    <td className="px-6 py-4">
                                        {task.status === 'PendingApproval' ? (
                                            <div className="flex items-center gap-2">
                                                <button onClick={() => onRequestAction(task.id, 'Reject')} className="p-2 text-red-500 hover:bg-red-100 rounded-full" title="رفض"><XCircleIcon className="w-5 h-5"/></button>
                                                <button onClick={() => onRequestAction(task.id, 'Approve')} className="p-2 text-emerald-500 hover:bg-emerald-100 rounded-full" title="موافقة"><CheckCircleIcon className="w-5 h-5"/></button>
                                            </div>
                                        ) : (
                                            <button onClick={() => handleOpenEditModal(task)} className="font-medium text-sky-600 hover:underline">
                                                تفاصيل
                                            </button>
                                        )}
                                    </td>
