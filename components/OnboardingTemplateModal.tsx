...
import { ONBOARDING_TASK_CATEGORIES, ONBOARDING_RESPONSIBLE_PARTIES } from '../constants';
...
interface OnboardingTemplateModalProps {
    ...
    templateToEdit: OnboardingTemplate | null;
}

const OnboardingTemplateModal: React.FC<OnboardingTemplateModalProps> = ({ isOpen, onClose, onSave, templateToEdit }) => {
    const { t } = useTranslation();
    const getInitialState = (): Omit<OnboardingTemplate, 'id'> => ({
        name: '',
        description: '',
        tasks: [{ title: '', category: 'paperwork', responsible: 'newEmployee', dueOffsetDays: 0 }]
    });
    ...
    const handleTaskChange = (index: number, field: keyof Omit<OnboardingTask, 'id' | 'isCompleted' | 'dueDate'>, value: string | number) => {
    ...
    const handleAddTask = () => {
        setTemplateData(prev => ({
            ...prev,
            tasks: [...prev.tasks, { title: '', category: 'paperwork', responsible: 'newEmployee', dueOffsetDays: 0 }]
        }));
    };
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        const finalTemplate: OnboardingTemplate = {
        ...
        >
            ...
            >
                <div className="flex justify-between items-center mb-6">
                    <h2 className="text-2xl font-bold text-slate-800">{templateToEdit ? 'تعديل قالب التعيين' : 'إنشاء قالب تعيين جديد'}</h2>
                        ...
                        {templateData.tasks.map((task, index) => (
                            <div key={index} className="p-3 bg-slate-50 rounded-lg border flex flex-col md:flex-row gap-2">
                                ...
                                <select value={task.category} onChange={e => handleTaskChange(index, 'category', e.target.value)} className="w-full md:w-48 p-1.5 border rounded bg-white text-sm">
                                    {ONBOARDING_TASK_CATEGORIES.map(cat => <option key={cat} value={cat}>{t(`onboarding.categories.${cat}`)}</option>)}
                                </select>
                                <select value={task.responsible} onChange={e => handleTaskChange(index, 'responsible', e.target.value)} className="w-full md:w-48 p-1.5 border rounded bg-white text-sm">
                                    {ONBOARDING_RESPONSIBLE_PARTIES.map(p => <option key={p} value={p}>{t(`onboarding.responsibles.${p}`)}</option>)}
                                </select>
                                <div className="flex items-center gap-1">
                                    ...
                                    />
                                    <span className="text-xs text-slate-600">أيام بعد البدء</span>
                                </div>
