...
import { OnboardingTemplate, OnboardingTask, OnboardingTaskCategory, OnboardingResponsible } from '../types';
...
const OnboardingTemplateModal: React.FC<OnboardingTemplateModalProps> = ({ isOpen, onClose, onSave, templateToEdit }) => {
    ...
    const handleAddTask = () => {
        const newTask: Omit<OnboardingTask, 'id' | 'isCompleted' | 'dueDate'> = {
            title: '',
            category: 'paperwork',
            responsible: 'newEmployee',
            dueOffsetDays: 0,
        };
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        onSave({ id: templateToEdit?.id || `ot-${Date.now()}`, ...template });
        onClose();
    };
                        ...
                        {template.tasks.map((task, index) => (
                            <div key={index} className="p-2 bg-slate-50 border rounded-md grid grid-cols-1 md:grid-cols-4 gap-2 items-center">
                                ...
                                <select value={task.category} onChange={e => handleTaskChange(index, 'category', e.target.value)} className="w-full p-1.5 border rounded bg-white">
                                    {ONBOARDING_TASK_CATEGORIES.map(c => <option key={c} value={c}>{t(`onboarding.categories.${c}`)}</option>)}
                                </select>
