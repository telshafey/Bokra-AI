            ...
            <div>
                     ...
                     {monthlyCheckIns.length > 0 ? (
                         <div className="space-y-4">
                             {monthlyCheckIns.map(checkIn => {
                                ...
                                const monthName = new Date(checkIn.year, checkIn.month).toLocaleString(locale, { month: 'long' });
                                 return(
                                    <div key={checkIn.id} className={`p-4 rounded-lg border-l-4 ${ratingStyle.border} ${ratingStyle.bg}`}>
                                        <div className="flex justify-between items-center">
                                            <p className="font-bold text-slate-800 dark:text-slate-200">{t('performancePage.checkinFor', { month: monthName, year: checkIn.year })}</p>
                                            <span className={`px-3 py-1 text-xs font-bold rounded-full ${ratingStyle.bg} ${ratingStyle.text}`}>{t(`performanceReview.ratings.${checkIn.rating}`)}</span>
                                        </div>
                         ...
                         <div className="text-center py-8 bg-white dark:bg-slate-800 rounded-xl shadow-sm">
                            <p className="text-slate-500 dark:text-slate-400">{t('performancePage.noCheckins')}</p>
                        </div>
                     )}
                 </div>
                 ...
                 {reviews.length > 0 ? (
                    ...
                    <div className="text-center py-16 bg-white dark:bg-slate-800 rounded-xl shadow-sm">
                        <h3 className="text-xl font-bold text-slate-700 dark:text-slate-200">{t('performancePage.noReviews')}</h3>
                        <p className="text-slate-500 dark:text-slate-400 mt-2">{t('performancePage.noReviewsSubtitle')}</p>
                    </div>
                 )}
            </div>
        </div>
