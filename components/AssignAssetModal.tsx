        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-4" onClick={onClose}>
            <div className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl p-8 w-full max-w-lg" onClick={e => e.stopPropagation()}>
                ...
                <div className="mb-4 p-3 bg-slate-50 dark:bg-slate-700 rounded-lg">
                    <p className="text-sm text-slate-600 dark:text-slate-300">{t('assignAssetModal.selectedAsset')}:</p>
                    <p className="font-bold text-slate-800 dark:text-slate-100">{asset.name} ({asset.serialNumber})</p>
                </div>
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">{t('assignAssetModal.assignTo')}</label>
                        <select
                        ...
                        >
                            <option value="">-- {t('assignAssetModal.returnToStock')} --</option>
                            {employees.map(emp => (
                    ...
                    <div className="flex justify-end gap-4 pt-4">
                        ...
                        <button type="submit" className="py-2 px-6 bg-emerald-600 text-white rounded-lg">{t('general.save')}</button>
                    </div>
                </form>
            </div>
        </div>
