...
interface AssignOvertimePolicyModalProps {
    isOpen: boolean;
    onClose: () => void;
...
const AssignOvertimePolicyModal: React.FC<AssignOvertimePolicyModalProps> = ({ isOpen, onClose, policy, employees, onAssign }) => {
    ...
    useEffect(() => {
        if (isOpen) {
            const initialSelection = new Set(
                employees.filter(e => e.overtimePolicyId === policy.id).map(e => e.id)
            );
    ...
    const filteredEmployees = useMemo(() => {
        return employees.filter(e => e.isEmployee && e.name.toLowerCase().includes(searchTerm.toLowerCase()));
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-4" onClick={onClose}>
            <div className="bg-white rounded-xl shadow-2xl p-8 w-full max-w-2xl flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
                <div className="flex justify-between items-center mb-4">
                    <h2 className="text-2xl font-bold text-slate-800">تعيين سياسة وقت إضافي: "{policy.name}"</h2>
...
export default AssignOvertimePolicyModal;
