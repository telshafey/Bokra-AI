...
interface OnboardingPageProps {
    ...
    onStartOnboarding: (employeeId: string, templateId: string, startDate: string) => void;
...
const OnboardingPage: React.FC<OnboardingPageProps> = ({ onboardingProcesses, onboardingTemplates, employees, onStartOnboarding, onUpdateTask }) => {
    ...
    }, [onboardingProcesses, employeeMap]);
    ...
    return (
        <div className="flex flex-col gap-6 h-[calc(100vh-120px)]">
            <div className="flex justify-between items-center">
                <div>
                    <h1 className="text-2xl font-bold text-slate-800">إدارة التعيينات الجديدة</h1>
                    <p className="text-slate-500">متابعة خطط انضمام الموظفين الجدد.</p>
                </div>
                <button
                ...
                >
                    ...
                    <span>بدء تعيين لموظف جديد</span>
                </button>
            </div>

            <div className="flex gap-6 flex-1 min-h-0">
                {/* Master: Onboarding List */}
                <div className="w-1/3 xl:w-1/4">
                    <div className="bg-white h-full rounded-xl shadow-md flex flex-col">
                        <div className="p-4 border-b">
                            <h2 className="text-xl font-bold text-slate-700">الموظفون الجدد ({enrichedProcesses.length})</h2>
                        </div>
                            ...
                            {enrichedProcesses.map(process => (
                                <button
                                    ...
                                    className={`w-full text-right p-4 transition-colors duration-200 border-r-4 ${
                                        selectedProcessId === process.id ? 'bg-sky-50 border-sky-500' : 'hover:bg-slate-50 border-transparent'
                                    }`}
                                >
                                    <div className="flex items-center gap-3">
                                        <img src={process.employee.avatarUrl} alt={process.employee.name} className="w-12 h-12 rounded-full" />
                                        <div className="flex-1">
                                            <p className={`font-semibold ${selectedProcessId === process.id ? 'text-sky-700' : 'text-slate-800'}`}>{process.employee.name}</p>
                ...
                {/* Detail: Onboarding Plan */}
                <div className="flex-1">
                    <div className="bg-white h-full rounded-xl shadow-md flex flex-col">
                        {selectedProcess ? (
                            <>
                                <div className="p-4 border-b">
                                    <h2 className="text-xl font-bold text-slate-800">خطة تعيين: {selectedProcess.employee.name}</h2>
                                    <p className="text-sm text-slate-500">تاريخ البدء: {new Date(selectedProcess.startDate).toLocaleDateString('ar-EG', { day: 'numeric', month: 'long', year: 'numeric' })}</p>
                                </div>
                            ...
                            <div className="flex items-center justify-center h-full text-slate-500">
                                <p>الرجاء اختيار موظف لعرض خطة التعيين الخاصة به.</p>
                            </div>
                        )}
                    </div>
                </div>
            ...
            <StartOnboardingModal
                ...
                activeProcessEmployeeIds={onboardingProcesses.map(p => p.employeeId)}
            />
        </div>
    );
};
