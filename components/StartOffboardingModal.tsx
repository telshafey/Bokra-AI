...
import { EmployeeProfile, OffboardingTemplate } from '../types';

interface StartOffboardingModalProps {
    isOpen: boolean;
    onClose: () => void;
    onStart: (employeeId: string, templateId: string, lastDay: string) => void;
    employees: EmployeeProfile[];
    templates: OffboardingTemplate[];
    activeProcessEmployeeIds: string[];
}

const StartOffboardingModal: React.FC<StartOffboardingModalProps> = ({ isOpen, onClose, onStart, employees, templates, activeProcessEmployeeIds }) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (!employeeId || !templateId || !lastDay) {
            alert('يرجى تعبئة جميع الحقول.');
            return;
        }
        onStart(employeeId, templateId, lastDay);
        onClose();
    };
        ...
        >
            ...
            >
                <div className="flex justify-between items-center mb-6">
                    <h2 className="text-2xl font-bold text-slate-800">بدء عملية إنهاء خدمة</h2>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div>
                        <label htmlFor="templateId" className="block text-sm font-medium text-slate-700 mb-1">اختر قالب إنهاء الخدمة</label>
                        <select
                            id="templateId"
                            value={templateId}
                    ...
                    <div>
                        <label htmlFor="lastDay" className="block text-sm font-medium text-slate-700 mb-1">آخر يوم عمل</label>
                        <input
                            type="date"
                            id="lastDay"
                            value={lastDay}
                    ...
                    <div className="flex justify-end gap-4 pt-4">
                        ...
                        <button type="submit" className="py-2 px-6 bg-red-600 text-white rounded-lg font-semibold hover:bg-red-700 shadow-sm">
                            بدء عملية إنهاء الخدمة
                        </button>
                    </div>
                </form>
            </div>
