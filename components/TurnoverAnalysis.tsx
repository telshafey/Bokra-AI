...
const TurnoverAnalysis: React.FC<{ employee: EmployeeProfile }> = ({ employee }) => {
    ...
    return (
        <div className="p-6">
            <h2 className="text-xl font-bold mb-4 text-slate-700">{t('turnover.title')}</h2>
            ...
            {analysis && (
                 <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className={`md:col-span-1 p-6 rounded-xl flex flex-col items-center justify-center text-center ${riskStyles.bg} border-2 ${riskStyles.border}`}>
                         <h3 className="text-sm font-semibold text-slate-600 mb-2">{t('turnover.expectedRiskLevel')}</h3>
                     ...
                     <div className="md:col-span-2 bg-slate-50 p-6 rounded-xl">
                         <h3 className="font-bold text-slate-800 mb-3">{t('turnover.keyFactors')}</h3>
                 ...
                 <div className="text-center p-8 border-2 border-dashed border-slate-300 rounded-xl">
                    {isLoading ? (
                        <div>
                            ...
                            <p className="font-semibold text-slate-700">{t('turnover.aiAnalyzing')}</p>
                            <p className="text-sm text-slate-500">{t('turnover.pleaseWait')}</p>
                        </div>
                    ) : (
                         <>
                            <PresentationChartLineIcon className="w-16 h-16 mx-auto text-slate-400 mb-4" />
                            <h3 className="text-lg font-bold text-slate-800">{t('turnover.proactiveAnalysis')}</h3>
                            <p className="text-slate-500 max-w-md mx-auto my-2">{t('turnover.description')}</p>
                            <button 
